...
import classes from "./ffYearnGauge.module.css";
...
export default function ffYearnGauge({ asset }) {
  ...
  useEffect(() => {
    ...
    stores.emitter.on(FIXED_FOREX_STAKE_YEARN_APPROVED, approveReturned);
    stores.emitter.on(FIXED_FOREX_YEARN_STAKED, depositReturned);
    stores.emitter.on(FIXED_FOREX_YEARN_UNSTAKED, depositReturned);
    stores.emitter.on(ERROR, errorReturned);
    return () => {
      stores.emitter.removeListener(FIXED_FOREX_STAKE_YEARN_APPROVED, approveReturned);
      stores.emitter.removeListener(FIXED_FOREX_YEARN_STAKED, depositReturned);
      stores.emitter.removeListener(FIXED_FOREX_YEARN_UNSTAKED, depositReturned);
  ...
  const onApprove = () => {
    ...
    stores.dispatcher.dispatch({ type: FIXED_FOREX_APPROVE_STAKE_YEARN, content: { asset, amount } });
  };

  const onDeposit = () => {
    setDepositLoading(true);
    stores.dispatcher.dispatch({ type: FIXED_FOREX_STAKE_YEARN, content: { asset, amount } });
  };

  const onWithdraw = () => {
    setDepositLoading(true);
    stores.dispatcher.dispatch({ type: FIXED_FOREX_UNSTAKE_YEARN, content: { asset, withdrawAmount } });
  };
  ...
  return (
    <div className={classes.container}>
      <div className={classes.inline}>
        <img className={classes.lpOptionIcon} src="/images/Yearn.png" alt="Yearn Logo" width={40} height={40} />
                          ...
                          <div className={classes.inputTitleContainer}>
                            ...
                            <div className={classes.balances}>
                              <Typography
                                ...
                                noWrap
                              >
                                Balance: {formatCurrency(asset && asset.yearn ? asset.yearn.userPoolBalance : 0)}
                              </Typography>
                            </div>
                          </div>
                          <TextField
                          ...
                          <div className={classes.inputTitleContainer}>
                            ...
                            <div className={classes.balances}>
                              <Typography
                                ...
                                noWrap
                              >
                                Balance: {formatCurrency(asset?.yearn?.userVaultBalance)}
                              </Typography>
                            </div>
                          </div>
                          <TextField
                    ...
                    <div className={classes.actionsContainer}>
                      ...
                      >
                        <Typography className={classes.actionButtonText}>
                          {depositApprovalNotRequired ? formatApproved(asset?.gauge?.poolGaugeAllowance) : approvalLoading ? `Approving` : `Approve`}
                        </Typography>
